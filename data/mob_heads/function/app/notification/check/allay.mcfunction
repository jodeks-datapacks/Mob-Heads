...
execute as @e[type=item,nbt={Item:{components:{"minecraft:note_block_sound":"minecraft:entity.allay.ambient_with_item","minecraft:custom_data":{mob_heads.checked_head:0b}}}}] \
  if items entity @s contents player_head run data modify storage mob_heads:root data.entity set value allay

execute as @e[type=item,nbt={Item:{components:{"minecraft:note_block_sound":"minecraft:entity.allay.ambient_with_item","minecraft:custom_data":{mob_heads.checked_head:0b}}}}] \
  if items entity @s contents player_head run function mob_heads:config/drop_messages with storage mob_heads:root data

execute as @e[type=item,nbt={Item:{components:{"minecraft:note_block_sound":"minecraft:entity.allay.ambient_with_item","minecraft:custom_data":{mob_heads.checked_head:0b}}}}] \
  if items entity @s contents player_head run scoreboard players add &allay mob_heads.head_dropped.mob 1
